...
it("should get correct sequence from seconds", () => {
  expect(getDigitsFromTime(552, getDigitsFromNumberContainer)).toEqual([0, 0]);
  expect(getDigitsFromTime(59235, getDigitsFromNumberContainer)).toEqual([
    5,
    9
  ]);
});
...
it("should get correct sequence from minutes and seconds", () => {
  ...
  expect(getDigitsFromTime(time, getDigitsFromNumberContainer)).toEqual(
    sequence
  );
